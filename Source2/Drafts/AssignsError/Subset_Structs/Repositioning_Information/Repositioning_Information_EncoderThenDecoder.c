...
*/
void Repositioning_Information_EncoderThenDecoder(Bitstream* stream, Repositioning_Information* p)
{
    const uint32_t length = REPOSITIONING_INFORMATION_BITSIZE;
    //@ ghost uint32_t pos = stream->bitpos;

    //@ assert Normal(stream, length);
    Repositioning_Information_Encoder(stream, p);
    ...
    */
    Repositioning_Information_Decoder(stream, p);
