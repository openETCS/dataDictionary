...
int Packet_DecodeInt(PacketHeader* header, const PacketInfo* data, const kcg_int* stream)
{
    switch (header->list)
    {
        case TRAINTOTRACK :
        {
            switch (header->NID_PACKET)
            {
                ...
                default :
                {
                    fprintf(stderr, "Packet_DecodeInt: unexpected value for header->NID_PACKET = %"PRIu64"\n", header->NID_PACKET);
                    return 0;
                }
            };
        }
        ...
        default:
        {
            fprintf(stderr, "Packet_DecodeInt: unexpected value for header->list = %"PRIu64"\n", header->list);
            return 0;
        }
    };
}
