...
inline int encode(PacketInfo& data, kcg_int* stream, const Stop_if_in_Staff_Responsible_Core& p)
{
    return Stop_if_in_Staff_Responsible_Encode_Int(&data, stream, &p);
}

inline int decode(PacketInfo& data, const kcg_int* stream, Stop_if_in_Staff_Responsible_Core& p)
{
