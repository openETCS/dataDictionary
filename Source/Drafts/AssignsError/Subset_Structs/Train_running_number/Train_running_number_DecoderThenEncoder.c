...
*/
void Train_running_number_DecoderThenEncoder(Bitstream* stream, Train_running_number* p)
{
    const uint32_t length = TRAIN_RUNNING_NUMBER_BITSIZE;
    //@ ghost uint32_t pos = stream->bitpos;

    //@ assert Normal(stream, length);
    Train_running_number_Decoder(stream, p);
    ...
    */
    Train_running_number_Encoder(stream, p);
