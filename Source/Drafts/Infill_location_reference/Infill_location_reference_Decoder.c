...
int Infill_location_reference_Decoder(Bitstream* stream, Infill_location_reference* p)
{
    if (NormalBitstream(stream, INFILL_LOCATION_REFERENCE_BITSIZE))
    {
	   ...
	   assigns         p->Q_DIR;
	   ensures  Q_DIR: stream->bitpos == pos + Q_DIR_E{Here}(p);
	   ensures  Q_DIR: EqualBits(stream, pos + Q_DIR_B{Here}(p), pos + Q_DIR_E{Here}(p), p->Q_DIR);
	   ensures  Q_DIR: UpperBitsNotSet(p->Q_DIR, 2);
	*/
	{ p->Q_DIR		= Bitstream_Read(stream, 2); }

        /*@
	   requires L_PACKET: stream->bitpos == pos + L_PACKET_B{Here}(p);
	   assigns            stream->bitpos;
	   assigns            p->L_PACKET;
	   ensures  L_PACKET: stream->bitpos == pos + L_PACKET_E{Here}(p);
	   ensures  L_PACKET: EqualBits(stream, pos + L_PACKET_B{Here}(p), pos + L_PACKET_E{Here}(p), p->L_PACKET);
	   ensures  L_PACKET: UpperBitsNotSet(p->L_PACKET, 13);
	*/
        { p->L_PACKET		= Bitstream_Read(stream, 13); }

        /*@
	   requires Q_NEWCOUNTRY: stream->bitpos == pos + Q_NEWCOUNTRY_B{Here}(p);
	   assigns                stream->bitpos;
	   assigns                p->Q_NEWCOUNTRY;
	   ensures  Q_NEWCOUNTRY: stream->bitpos == pos + Q_NEWCOUNTRY_E{Here}(p);
	   ensures  Q_NEWCOUNTRY: EqualBits(stream, pos + Q_NEWCOUNTRY_B{Here}(p), pos + Q_NEWCOUNTRY_E{Here}(p), p->Q_NEWCOUNTRY);
	...
	*/
	...
	/*@
	   ...
	   ensures  NID_C: UpperBitsNotSet(p->NID_C, NID_C_E{Here}(p) - NID_C_B{Here}(p));
	   ...
	   behavior read:
	     ...
	     ensures  NID_C: NID_C_E{Here}(p) == NID_C_B{Here}(p) + 10;
	   ...
	   behavior unread:
	     ...
	     ensures  NID_C: NID_C_E{Here}(p) == NID_C_B{Here}(p);
	...
	*/
	...
	//@ assert Q_DIR:            EqualBits(stream, pos + Q_DIR_B{Here}(p), pos + Q_DIR_E{Here}(p), p->Q_DIR);
	//@ assert Q_DIR:            UpperBitsNotSet(p->Q_DIR, 2);

	//@ assert L_PACKET:         EqualBits(stream, pos + L_PACKET_B{Here}(p), pos + L_PACKET_E{Here}(p), p->L_PACKET);
