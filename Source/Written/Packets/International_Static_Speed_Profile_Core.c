...
int International_Static_Speed_Profile_Encode_Int(PacketInfo* data, kcg_int* stream, const International_Static_Speed_Profile_Core* p)
{
    ...
    for (uint32_t i = 0; i < p->N_ITER_1; ++i)
    {
        International_Static_Speed_Profile_Core_1_Encode_Int(data, stream, &(p->sub_1[i]));
    }

    for (uint32_t i = 0; i < p->N_ITER_2; ++i)
    {
        International_Static_Speed_Profile_Core_2_Encode_Int(data, stream, &(p->sub_2[i]));
    }
...
int International_Static_Speed_Profile_Decode_Int(PacketInfo* data, const kcg_int* stream, International_Static_Speed_Profile_Core* p)
{
    ...
    for (uint32_t i = 0; i < p->N_ITER_1; ++i)
    {
        International_Static_Speed_Profile_Core_1_Decode_Int(data, stream, &(p->sub_1[i]));
    }

    for (uint32_t i = 0; i < p->N_ITER_2; ++i)
    {
        International_Static_Speed_Profile_Core_2_Decode_Int(data, stream, &(p->sub_2[i]));
    }
