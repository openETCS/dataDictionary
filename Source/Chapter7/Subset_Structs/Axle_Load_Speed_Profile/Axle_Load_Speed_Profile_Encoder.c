...
int Axle_Load_Speed_Profile_Encoder(Bitstream* stream, const Axle_Load_Speed_Profile* p)
{
    if (NormalBitstream(stream, AXLE_LOAD_SPEED_PROFILE_BITSIZE))
    {
        if (Axle_Load_Speed_Profile_UpperBitsNotSet(p))
        {
            ...
            Bitwalker_Poke_Normal(addr, size, pos + 41,  15, p->D_AXLELOAD);
            Bitwalker_Poke_Normal(addr, size, pos + 56,  15, p->L_AXLELOAD);
            Bitwalker_Poke_Normal(addr, size, pos + 71,  1,  p->Q_FRONT);
            Bitwalker_Poke_Normal(addr, size, pos + 72,  5,  p->N_ITER);
            Bitwalker_Poke_Normal(addr, size, pos + 77,  7,  p->M_AXLELOADCAT(n));
            Bitwalker_Poke_Normal(addr, size, pos + 84,  7,  p->V_AXLELOAD(n));
            Bitwalker_Poke_Normal(addr, size, pos + 91,  5,  p->N_ITER);
            Bitwalker_Poke_Normal(addr, size, pos + 96,  15, p->D_AXLELOAD(k));
            Bitwalker_Poke_Normal(addr, size, pos + 111, 15, p->L_AXLELOAD(k));
            Bitwalker_Poke_Normal(addr, size, pos + 126, 1,  p->Q_FRONT(k));
            Bitwalker_Poke_Normal(addr, size, pos + 127, 5,  p->N_ITER(k));
            Bitwalker_Poke_Normal(addr, size, pos + 132, 7,  p->M_AXLELOADCAT(k,m));
            Bitwalker_Poke_Normal(addr, size, pos + 139, 7,  p->V_AXLELOAD(k,m));
