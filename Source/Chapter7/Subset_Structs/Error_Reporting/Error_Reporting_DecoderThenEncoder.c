...
*/
void Error_Reporting_DecoderThenEncoder(Bitstream* stream, Error_Reporting* p)
{
    const uint32_t length = ERROR_REPORTING_BITSIZE;
    //@ ghost uint32_t pos = stream->bitpos;

    //@ assert Normal(stream, length);
    Error_Reporting_Decoder(stream, p);
    ...
    Error_Reporting_Encoder(stream, p);
