...
int Position_Report_Parameters_Encoder(Bitstream* stream, const Position_Report_Parameters* p)
{
    if (NormalBitstream(stream, POSITION_REPORT_PARAMETERS_BITSIZE))
    {
        if (Position_Report_Parameters_UpperBitsNotSet(p))
        {
            ...
            Bitwalker_Poke_Normal(addr, size, pos + 25,  8,  p->T_CYCLOC);
            Bitwalker_Poke_Normal(addr, size, pos + 33,  15, p->D_CYCLOC);
            Bitwalker_Poke_Normal(addr, size, pos + 48,  3,  p->M_LOC);
            Bitwalker_Poke_Normal(addr, size, pos + 51,  5,  p->N_ITER);
            Bitwalker_Poke_Normal(addr, size, pos + 56,  15, p->D_LOC(k));
            Bitwalker_Poke_Normal(addr, size, pos + 71,  1,  p->Q_LGTLOC(k));
