...
int International_Static_Speed_Profile_Decoder(Bitstream* stream, International_Static_Speed_Profile* p)
{
    if (NormalBitstream(stream, INTERNATIONAL_STATIC_SPEED_PROFILE_BITSIZE))
    {
        ...
        p->D_STATIC           = Bitwalker_Peek_Normal(addr, size, pos + 25,  15);
        p->V_STATIC           = Bitwalker_Peek_Normal(addr, size, pos + 40,  7);
        p->Q_FRONT            = Bitwalker_Peek_Normal(addr, size, pos + 47,  1);
        p->N_ITER             = Bitwalker_Peek_Normal(addr, size, pos + 48,  5);
        p->Q_DIFF(n)          = Bitwalker_Peek_Normal(addr, size, pos + 53,  2);
        p->NC_CDDIFF(n)       = Bitwalker_Peek_Normal(addr, size, pos + 55,  4);
        p->NC_DIFF(n)         = Bitwalker_Peek_Normal(addr, size, pos + 59,  4);
        p->V_DIFF(n)          = Bitwalker_Peek_Normal(addr, size, pos + 63,  7);
        p->N_ITER             = Bitwalker_Peek_Normal(addr, size, pos + 70,  5);
        p->D_STATIC(k)        = Bitwalker_Peek_Normal(addr, size, pos + 75,  15);
        p->V_STATIC(k)        = Bitwalker_Peek_Normal(addr, size, pos + 90,  7);
        p->Q_FRONT(k)         = Bitwalker_Peek_Normal(addr, size, pos + 97,  1);
        p->N_ITER(k)          = Bitwalker_Peek_Normal(addr, size, pos + 98,  5);
        p->Q_DIFF(k,m)        = Bitwalker_Peek_Normal(addr, size, pos + 103, 2);
        p->NC_CDDIFF(k,m)     = Bitwalker_Peek_Normal(addr, size, pos + 105, 4);
        p->NC_DIFF(k,m)       = Bitwalker_Peek_Normal(addr, size, pos + 109, 4);
        p->V_DIFF(k,m)        = Bitwalker_Peek_Normal(addr, size, pos + 113, 7);
