...
int Level_23_Movement_Authority_Decoder(Bitstream* stream, Level_23_Movement_Authority* p)
{
    if (NormalBitstream(stream, LEVEL_23_MOVEMENT_AUTHORITY_BITSIZE))
    {
        ...
        p->V_LOA              = Bitwalker_Peek_Normal(addr, size, pos + 25,  7);
        p->T_LOA              = Bitwalker_Peek_Normal(addr, size, pos + 32,  10);
        p->N_ITER             = Bitwalker_Peek_Normal(addr, size, pos + 42,  5);
        p->L_SECTION(k)       = Bitwalker_Peek_Normal(addr, size, pos + 47,  15);
        p->Q_SECTIONTIMER(k)  = Bitwalker_Peek_Normal(addr, size, pos + 62,  1);
        p->T_SECTIONTIMER(k)  = Bitwalker_Peek_Normal(addr, size, pos + 63,  10);
        p->D_SECTIONTIMERSTOPLOC(k) = Bitwalker_Peek_Normal(addr, size, pos + 73,  15);
        p->L_ENDSECTION       = Bitwalker_Peek_Normal(addr, size, pos + 88,  15);
        p->Q_SECTIONTIMER     = Bitwalker_Peek_Normal(addr, size, pos + 103, 1);
        p->T_SECTIONTIMER     = Bitwalker_Peek_Normal(addr, size, pos + 104, 10);
        p->D_SECTIONTIMERSTOPLOC = Bitwalker_Peek_Normal(addr, size, pos + 114, 15);
        p->Q_ENDTIMER         = Bitwalker_Peek_Normal(addr, size, pos + 129, 1);
        p->T_ENDTIMER         = Bitwalker_Peek_Normal(addr, size, pos + 130, 10);
        p->D_ENDTIMERSTARTLOC = Bitwalker_Peek_Normal(addr, size, pos + 140, 15);
        p->Q_DANGERPOINT      = Bitwalker_Peek_Normal(addr, size, pos + 155, 1);
        p->D_DP               = Bitwalker_Peek_Normal(addr, size, pos + 156, 15);
        p->V_RELEASEDP        = Bitwalker_Peek_Normal(addr, size, pos + 171, 7);
        p->Q_OVERLAP          = Bitwalker_Peek_Normal(addr, size, pos + 178, 1);
        p->D_STARTOL          = Bitwalker_Peek_Normal(addr, size, pos + 179, 15);
        p->T_OL               = Bitwalker_Peek_Normal(addr, size, pos + 194, 10);
        p->D_OL               = Bitwalker_Peek_Normal(addr, size, pos + 204, 15);
        p->V_RELEASEOL        = Bitwalker_Peek_Normal(addr, size, pos + 219, 7);
