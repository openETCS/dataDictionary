...
int Level_23_Movement_Authority_UpperBitsNotSet(const Level_23_Movement_Authority* p)
{
    if (UpperBitsNotSet64(p->NID_PACKET,        8)   &&
        ...
        UpperBitsNotSet64(p->L_SECTION(k),      15)  &&
        UpperBitsNotSet64(p->Q_SECTIONTIMER(k), 1)   &&
        UpperBitsNotSet64(p->T_SECTIONTIMER(k), 10)  &&
        UpperBitsNotSet64(p->D_SECTIONTIMERSTOPLOC(k), 15)  &&
