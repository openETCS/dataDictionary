...
*/
void Danger_for_Shunting_information_EncoderThenDecoder(Bitstream* stream, Danger_for_Shunting_information* p)
{
    const uint32_t length = DANGER_FOR_SHUNTING_INFORMATION_BITSIZE;
    //@ ghost uint32_t pos = stream->bitpos;

    //@ assert Normal(stream, length);
    Danger_for_Shunting_information_Encoder(stream, p);
    ...
    Danger_for_Shunting_information_Decoder(stream, p);
