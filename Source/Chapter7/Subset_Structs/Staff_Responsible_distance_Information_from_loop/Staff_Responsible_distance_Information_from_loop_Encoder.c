...
int Staff_Responsible_distance_Information_from_loop_Encoder(Bitstream* stream, const Staff_Responsible_distance_Information_from_loop* p)
{
    if (NormalBitstream(stream, STAFF_RESPONSIBLE_DISTANCE_INFORMATION_FROM_LOOP_BITSIZE))
    {
        if (Staff_Responsible_distance_Information_from_loop_UpperBitsNotSet(p))
        {
            ...
            Bitwalker_Poke_Normal(addr, size, pos + 25,  1,  p->Q_NEWCOUNTRY(0));
            Bitwalker_Poke_Normal(addr, size, pos + 26,  10, p->NID_C(0));
            Bitwalker_Poke_Normal(addr, size, pos + 36,  14, p->NID_BG(0));
            Bitwalker_Poke_Normal(addr, size, pos + 50,  1,  p->Q_NEWCOUNTRY(1));
            Bitwalker_Poke_Normal(addr, size, pos + 51,  10, p->NID_C(1));
            Bitwalker_Poke_Normal(addr, size, pos + 61,  14, p->NID_BG(1));
            Bitwalker_Poke_Normal(addr, size, pos + 75,  15, p->D_SR);
            Bitwalker_Poke_Normal(addr, size, pos + 90,  5,  p->N_ITER);
            Bitwalker_Poke_Normal(addr, size, pos + 95,  1,  p->Q_NEWCOUNTRY_(k));
            Bitwalker_Poke_Normal(addr, size, pos + 96,  10, p->NID_C_(k));
            Bitwalker_Poke_Normal(addr, size, pos + 106, 14, p->NID_BG_(k));
            Bitwalker_Poke_Normal(addr, size, pos + 120, 15, p->D_SR_(k));
