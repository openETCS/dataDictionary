...
struct Packet_for_sending_fixed_text_messages
{
...
typedef struct Packet_for_sending_fixed_text_messages Packet_for_sending_fixed_text_messages;

#define PACKET_FOR_SENDING_FIXED_TEXT_MESSAGES_BITSIZE 142

/*@
    logic integer BitSize{L}(Packet_for_sending_fixed_text_messages* p) = PACKET_FOR_SENDING_FIXED_TEXT_MESSAGES_BITSIZE;

    logic integer MaxBitSize{L}(Packet_for_sending_fixed_text_messages* p) = BitSize(p);

    predicate Separated(Bitstream* stream, Packet_for_sending_fixed_text_messages* p) =
      \separated(stream, p) &&
      \separated(stream->addr + (0..stream->size-1), p);

    predicate Invariant(Packet_for_sending_fixed_text_messages* p) =
    ...
    predicate ZeroInitialized(Packet_for_sending_fixed_text_messages* p) =
    ...
    predicate EqualBits(Bitstream* stream, integer pos, Packet_for_sending_fixed_text_messages* p) =
      ...
      EqualBits(stream, pos + 134, pos + 142, p->Q_TEXT);


    predicate UpperBitsNotSet(Packet_for_sending_fixed_text_messages* p) =
