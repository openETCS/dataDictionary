...
int Geographical_Position_Information_Decoder(Bitstream* stream, Geographical_Position_Information* p)
{
    if (NormalBitstream(stream, GEOGRAPHICAL_POSITION_INFORMATION_BITSIZE))
    {
        ...
        p->Q_NEWCOUNTRY       = Bitwalker_Peek_Normal(addr, size, pos + 25,  1);
        p->NID_C              = Bitwalker_Peek_Normal(addr, size, pos + 26,  10);
        p->NID_BG             = Bitwalker_Peek_Normal(addr, size, pos + 36,  14);
        p->D_POSOFF           = Bitwalker_Peek_Normal(addr, size, pos + 50,  15);
        p->Q_MPOSITION        = Bitwalker_Peek_Normal(addr, size, pos + 65,  1);
        p->M_POSITION         = Bitwalker_Peek_Normal(addr, size, pos + 66,  24);
        p->N_ITER             = Bitwalker_Peek_Normal(addr, size, pos + 90,  5);
        p->Q_NEWCOUNTRY(k)    = Bitwalker_Peek_Normal(addr, size, pos + 95,  1);
        p->NID_C(k)           = Bitwalker_Peek_Normal(addr, size, pos + 96,  10);
        p->NID_BG(k)          = Bitwalker_Peek_Normal(addr, size, pos + 106, 14);
        p->D_POSOFF(k)        = Bitwalker_Peek_Normal(addr, size, pos + 120, 15);
        p->Q_MPOSITION(k)     = Bitwalker_Peek_Normal(addr, size, pos + 135, 1);
        p->M_POSITION(k)      = Bitwalker_Peek_Normal(addr, size, pos + 136, 24);
