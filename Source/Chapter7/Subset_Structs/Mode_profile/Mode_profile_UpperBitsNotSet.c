...
int Mode_profile_UpperBitsNotSet(const Mode_profile* p)
{
    if (UpperBitsNotSet64(p->NID_PACKET,        8)   &&
        ...
        UpperBitsNotSet64(p->D_MAMODE(k),       15)  &&
        UpperBitsNotSet64(p->M_MAMODE(k),       2)   &&
        UpperBitsNotSet64(p->V_MAMODE(k),       7)   &&
        UpperBitsNotSet64(p->L_MAMODE(k),       15)  &&
        UpperBitsNotSet64(p->L_ACKMAMODE(k),    15)  &&
        UpperBitsNotSet64(p->Q_MAMODE(k),       1))
    {
        return 1;
    }
    else
