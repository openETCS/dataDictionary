...
int Linking_Encoder(Bitstream* stream, const Linking* p)
{
    if (NormalBitstream(stream, LINKING_BITSIZE))
    {
        if (Linking_UpperBitsNotSet(p))
        {
            ...
            Bitwalker_Poke_Normal(addr, size, pos + 25,  15, p->D_LINK);
            Bitwalker_Poke_Normal(addr, size, pos + 40,  1,  p->Q_NEWCOUNTRY);
            Bitwalker_Poke_Normal(addr, size, pos + 41,  10, p->NID_C);
            Bitwalker_Poke_Normal(addr, size, pos + 51,  14, p->NID_BG);
            Bitwalker_Poke_Normal(addr, size, pos + 65,  1,  p->Q_LINKORIENTATION);
            Bitwalker_Poke_Normal(addr, size, pos + 66,  2,  p->Q_LINKREACTION);
            Bitwalker_Poke_Normal(addr, size, pos + 68,  6,  p->Q_LOCACC);
            Bitwalker_Poke_Normal(addr, size, pos + 74,  5,  p->N_ITER);
            Bitwalker_Poke_Normal(addr, size, pos + 79,  15, p->D_LINK_(k));
            Bitwalker_Poke_Normal(addr, size, pos + 94,  1,  p->Q_NEWCOUNTRY(k));
            Bitwalker_Poke_Normal(addr, size, pos + 95,  10, p->NID_C_(k));
            Bitwalker_Poke_Normal(addr, size, pos + 105, 14, p->NID_BG_(k));
            Bitwalker_Poke_Normal(addr, size, pos + 119, 1,  p->Q_LINKORIENTATION_(k));
            Bitwalker_Poke_Normal(addr, size, pos + 120, 2,  p->Q_LINKREACTION_(k));
            Bitwalker_Poke_Normal(addr, size, pos + 122, 6,  p->Q_LOCACC_(k));
