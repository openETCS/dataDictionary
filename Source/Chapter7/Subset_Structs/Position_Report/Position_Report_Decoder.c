...
int Position_Report_Decoder(Bitstream* stream, Position_Report* p)
{
    if (NormalBitstream(stream, POSITION_REPORT_BITSIZE))
    {
        ...
        p->D_LRBG             = Bitwalker_Peek_Normal(addr, size, pos + 47,  15);
        p->Q_DIRLRBG          = Bitwalker_Peek_Normal(addr, size, pos + 62,  2);
        p->Q_DLRBG            = Bitwalker_Peek_Normal(addr, size, pos + 64,  2);
        p->L_DOUBTOVER        = Bitwalker_Peek_Normal(addr, size, pos + 66,  15);
        p->L_DOUBTUNDER       = Bitwalker_Peek_Normal(addr, size, pos + 81,  15);
        p->Q_LENGTH           = Bitwalker_Peek_Normal(addr, size, pos + 96,  2);
        p->L_TRAININT         = Bitwalker_Peek_Normal(addr, size, pos + 98,  15);
        p->V_TRAIN            = Bitwalker_Peek_Normal(addr, size, pos + 113, 7);
        p->Q_DIRTRAIN         = Bitwalker_Peek_Normal(addr, size, pos + 120, 2);
        p->M_MODE             = Bitwalker_Peek_Normal(addr, size, pos + 122, 4);
        p->M_LEVEL            = Bitwalker_Peek_Normal(addr, size, pos + 126, 3);
        p->NID_NTC            = Bitwalker_Peek_Normal(addr, size, pos + 129, 8);
