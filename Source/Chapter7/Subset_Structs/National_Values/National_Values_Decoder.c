...
int National_Values_Decoder(Bitstream* stream, National_Values* p)
{
    if (NormalBitstream(stream, NATIONAL_VALUES_BITSIZE))
    {
        ...
        p->D_VALIDNV          = Bitwalker_Peek_Normal(addr, size, pos + 25,  15);
        p->NID_C              = Bitwalker_Peek_Normal(addr, size, pos + 40,  10);
        p->N_ITER             = Bitwalker_Peek_Normal(addr, size, pos + 50,  5);
        p->NID_C(p)           = Bitwalker_Peek_Normal(addr, size, pos + 55,  10);
        p->V_NVSHUNT          = Bitwalker_Peek_Normal(addr, size, pos + 65,  7);
        p->V_NVSTFF           = Bitwalker_Peek_Normal(addr, size, pos + 72,  7);
        p->V_NVONSIGHT        = Bitwalker_Peek_Normal(addr, size, pos + 79,  7);
        p->V_NVLIMSUPERV      = Bitwalker_Peek_Normal(addr, size, pos + 86,  7);
        p->V_NVUNFIT          = Bitwalker_Peek_Normal(addr, size, pos + 93,  7);
        p->V_NVREL            = Bitwalker_Peek_Normal(addr, size, pos + 100, 7);
        p->D_NVROLL           = Bitwalker_Peek_Normal(addr, size, pos + 107, 15);
        p->Q_NVSBTSMPERM      = Bitwalker_Peek_Normal(addr, size, pos + 122, 1);
        p->Q_NVEMRRLS         = Bitwalker_Peek_Normal(addr, size, pos + 123, 1);
        p->Q_NVGUIPERM        = Bitwalker_Peek_Normal(addr, size, pos + 124, 1);
        p->Q_NVSBFBPERM       = Bitwalker_Peek_Normal(addr, size, pos + 125, 1);
        p->Q_NVINHSMICPERM    = Bitwalker_Peek_Normal(addr, size, pos + 126, 1);
        p->V_NVALLOWOVTRP     = Bitwalker_Peek_Normal(addr, size, pos + 127, 7);
        p->V_NVSUPOVTRP       = Bitwalker_Peek_Normal(addr, size, pos + 134, 7);
        p->D_NVOVTRP          = Bitwalker_Peek_Normal(addr, size, pos + 141, 15);
        p->T_NVOVTRP          = Bitwalker_Peek_Normal(addr, size, pos + 156, 8);
        p->D_NVPOTRP          = Bitwalker_Peek_Normal(addr, size, pos + 164, 15);
        p->M_NVCONTACT        = Bitwalker_Peek_Normal(addr, size, pos + 179, 2);
        p->T_NVCONTACT        = Bitwalker_Peek_Normal(addr, size, pos + 181, 8);
        p->M_NVDERUN          = Bitwalker_Peek_Normal(addr, size, pos + 189, 1);
        p->D_NVSTFF           = Bitwalker_Peek_Normal(addr, size, pos + 190, 15);
        p->Q_NVDRIVER_ADHES   = Bitwalker_Peek_Normal(addr, size, pos + 205, 1);
        p->A_NVMAXREDADH1     = Bitwalker_Peek_Normal(addr, size, pos + 206, 6);
        p->A_NVMAXREDADH2     = Bitwalker_Peek_Normal(addr, size, pos + 212, 6);
        p->A_NVMAXREDADH3     = Bitwalker_Peek_Normal(addr, size, pos + 218, 6);
        p->Q_NVLOCACC         = Bitwalker_Peek_Normal(addr, size, pos + 224, 6);
        p->M_NVAVADH          = Bitwalker_Peek_Normal(addr, size, pos + 230, 5);
        p->M_NVEBCL           = Bitwalker_Peek_Normal(addr, size, pos + 235, 4);
        p->Q_NVKINT           = Bitwalker_Peek_Normal(addr, size, pos + 239, 1);
        p->Q_NVKVINTSET       = Bitwalker_Peek_Normal(addr, size, pos + 240, 2);
        p->A_NVP12            = Bitwalker_Peek_Normal(addr, size, pos + 242, 6);
        p->A_NVP23            = Bitwalker_Peek_Normal(addr, size, pos + 248, 6);
        p->V_NVKVINT          = Bitwalker_Peek_Normal(addr, size, pos + 254, 7);
        p->M_NVKVINT          = Bitwalker_Peek_Normal(addr, size, pos + 261, 7);
        p->N_ITER             = Bitwalker_Peek_Normal(addr, size, pos + 268, 5);
        p->V_NVKVINT(n)       = Bitwalker_Peek_Normal(addr, size, pos + 273, 7);
        p->M_NVKVINT(n)       = Bitwalker_Peek_Normal(addr, size, pos + 280, 7);
        p->N_ITER             = Bitwalker_Peek_Normal(addr, size, pos + 287, 5);
        p->Q_NVKVINTSET(k)    = Bitwalker_Peek_Normal(addr, size, pos + 292, 2);
        p->A_NVP12(k)         = Bitwalker_Peek_Normal(addr, size, pos + 294, 6);
        p->A_NVP23(k)         = Bitwalker_Peek_Normal(addr, size, pos + 300, 6);
        p->V_NVKVINT(k)       = Bitwalker_Peek_Normal(addr, size, pos + 306, 7);
        p->M_NVKVINT(k)       = Bitwalker_Peek_Normal(addr, size, pos + 313, 7);
        p->N_ITER(k)          = Bitwalker_Peek_Normal(addr, size, pos + 320, 5);
        p->V_NVKVINT(k,m)     = Bitwalker_Peek_Normal(addr, size, pos + 325, 7);
        p->M_NVKVINT(k,m)     = Bitwalker_Peek_Normal(addr, size, pos + 332, 7);
        p->L_NVKRINT          = Bitwalker_Peek_Normal(addr, size, pos + 339, 5);
        p->M_NVKRINT          = Bitwalker_Peek_Normal(addr, size, pos + 344, 5);
        p->N_ITER             = Bitwalker_Peek_Normal(addr, size, pos + 349, 5);
        p->L_NVKRINT(l)       = Bitwalker_Peek_Normal(addr, size, pos + 354, 5);
        p->M_NVKRINT(l)       = Bitwalker_Peek_Normal(addr, size, pos + 359, 5);
        p->M_NVKTINT          = Bitwalker_Peek_Normal(addr, size, pos + 364, 5);
