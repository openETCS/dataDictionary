...
int Track_Condition_Station_Platforms_Encoder(Bitstream* stream, const Track_Condition_Station_Platforms* p)
{
    if (NormalBitstream(stream, TRACK_CONDITION_STATION_PLATFORMS_BITSIZE))
    {
        if (Track_Condition_Station_Platforms_UpperBitsNotSet(p))
        {
            ...
            Bitwalker_Poke_Normal(addr, size, pos + 71,  4,  p->M_PLATFORM);
            Bitwalker_Poke_Normal(addr, size, pos + 75,  2,  p->Q_PLATFORM);
            Bitwalker_Poke_Normal(addr, size, pos + 77,  5,  p->N_ITER);
            Bitwalker_Poke_Normal(addr, size, pos + 82,  15, p->D_TRACKCOND(k));
            Bitwalker_Poke_Normal(addr, size, pos + 97,  15, p->L_TRACKCOND(k));
            Bitwalker_Poke_Normal(addr, size, pos + 112, 4,  p->M_PLATFORM(k));
            Bitwalker_Poke_Normal(addr, size, pos + 116, 2,  p->Q_PLATFORM(k));
