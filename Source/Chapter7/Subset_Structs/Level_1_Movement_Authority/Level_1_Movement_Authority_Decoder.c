...
#include "Level_1_Movement_Authority_Decoder.h"
#include "Bitwalker_Peek_Normal.h"

int Level_1_Movement_Authority_Decoder(Bitstream* stream, Level_1_Movement_Authority* p)
{
    if (NormalBitstream(stream, LEVEL_1_MOVEMENT_AUTHORITY_BITSIZE))
    {
        ...
        p->V_MAIN             = Bitwalker_Peek_Normal(addr, size, pos + 25,  7);
        p->V_LOA              = Bitwalker_Peek_Normal(addr, size, pos + 32,  7);
        p->T_LOA              = Bitwalker_Peek_Normal(addr, size, pos + 39,  10);
        p->N_ITER             = Bitwalker_Peek_Normal(addr, size, pos + 49,  5);
        p->L_SECTION(k)       = Bitwalker_Peek_Normal(addr, size, pos + 54,  15);
        p->Q_SECTIONTIMER(k)  = Bitwalker_Peek_Normal(addr, size, pos + 69,  1);
        p->T_SECTIONTIMER(k)  = Bitwalker_Peek_Normal(addr, size, pos + 70,  10);
        p->D_SECTIONTIMERSTOPLOC(k) = Bitwalker_Peek_Normal(addr, size, pos + 80,  15);
        p->L_ENDSECTION       = Bitwalker_Peek_Normal(addr, size, pos + 95,  15);
        p->Q_SECTIONTIMER     = Bitwalker_Peek_Normal(addr, size, pos + 110, 1);
        p->T_SECTIONTIMER     = Bitwalker_Peek_Normal(addr, size, pos + 111, 10);
        p->D_SECTIONTIMERSTOPLOC = Bitwalker_Peek_Normal(addr, size, pos + 121, 15);
        p->Q_ENDTIMER         = Bitwalker_Peek_Normal(addr, size, pos + 136, 1);
        p->T_ENDTIMER         = Bitwalker_Peek_Normal(addr, size, pos + 137, 10);
        p->D_ENDTIMERSTARTLOC = Bitwalker_Peek_Normal(addr, size, pos + 147, 15);
        p->Q_DANGERPOINT      = Bitwalker_Peek_Normal(addr, size, pos + 162, 1);
        p->D_DP               = Bitwalker_Peek_Normal(addr, size, pos + 163, 15);
        p->V_RELEASEDP        = Bitwalker_Peek_Normal(addr, size, pos + 178, 7);
        p->Q_OVERLAP          = Bitwalker_Peek_Normal(addr, size, pos + 185, 1);
        p->D_STARTOL          = Bitwalker_Peek_Normal(addr, size, pos + 186, 15);
        p->T_OL               = Bitwalker_Peek_Normal(addr, size, pos + 201, 10);
        p->D_OL               = Bitwalker_Peek_Normal(addr, size, pos + 211, 15);
        p->V_RELEASEOL        = Bitwalker_Peek_Normal(addr, size, pos + 226, 7);
