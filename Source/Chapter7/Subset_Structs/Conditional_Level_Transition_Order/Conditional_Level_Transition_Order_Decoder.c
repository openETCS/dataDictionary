...
int Conditional_Level_Transition_Order_Decoder(Bitstream* stream, Conditional_Level_Transition_Order* p)
{
    if (NormalBitstream(stream, CONDITIONAL_LEVEL_TRANSITION_ORDER_BITSIZE))
    {
        ...
        p->M_LEVELTR          = Bitwalker_Peek_Normal(addr, size, pos + 23,  3);
        p->NID_NTC            = Bitwalker_Peek_Normal(addr, size, pos + 26,  8);
        p->N_ITER             = Bitwalker_Peek_Normal(addr, size, pos + 34,  5);
        p->M_LEVELTR(k)       = Bitwalker_Peek_Normal(addr, size, pos + 39,  3);
        p->NID_NTC(k)         = Bitwalker_Peek_Normal(addr, size, pos + 42,  8);
