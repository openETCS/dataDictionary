...
int Position_Report_based_on_two_balise_groups_Encoder(Bitstream* stream, const Position_Report_based_on_two_balise_groups* p)
{
    if (NormalBitstream(stream, POSITION_REPORT_BASED_ON_TWO_BALISE_GROUPS_BITSIZE))
    {
        if (Position_Report_based_on_two_balise_groups_UpperBitsNotSet(p))
        {
            ...
            Bitwalker_Poke_Normal(addr, size, pos + 47,  24, p->NID_PRVLRBG);
            Bitwalker_Poke_Normal(addr, size, pos + 71,  15, p->D_LRBG);
            Bitwalker_Poke_Normal(addr, size, pos + 86,  2,  p->Q_DIRLRBG);
            Bitwalker_Poke_Normal(addr, size, pos + 88,  2,  p->Q_DLRBG);
            Bitwalker_Poke_Normal(addr, size, pos + 90,  15, p->L_DOUBTOVER);
            Bitwalker_Poke_Normal(addr, size, pos + 105, 15, p->L_DOUBTUNDER);
            Bitwalker_Poke_Normal(addr, size, pos + 120, 2,  p->Q_LENGTH);
            Bitwalker_Poke_Normal(addr, size, pos + 122, 15, p->L_TRAININT);
            Bitwalker_Poke_Normal(addr, size, pos + 137, 7,  p->V_TRAIN);
            Bitwalker_Poke_Normal(addr, size, pos + 144, 2,  p->Q_DIRTRAIN);
            Bitwalker_Poke_Normal(addr, size, pos + 146, 4,  p->M_MODE);
            Bitwalker_Poke_Normal(addr, size, pos + 150, 3,  p->M_LEVEL);
            Bitwalker_Poke_Normal(addr, size, pos + 153, 8,  p->NID_NTC);
