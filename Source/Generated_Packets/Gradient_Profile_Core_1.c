...
int Gradient_Profile_Core_1_Encoder(Bitstream* stream, const Gradient_Profile_Core_1* p)
{
    if (NormalBitstream(stream, GRADIENT_PROFILE_CORE_1_CORE_BITSIZE))
    {
        if (Gradient_Profile_Core_1_UpperBitsNotSet(p))
        {
            ...
            //@ assert D_GRADIENT_k:      EqualBits(stream, pos + 46,  pos + 61,  p->D_GRADIENT_k);
            //@ assert Q_GDIR_k:          EqualBits(stream, pos + 61,  pos + 62,  p->Q_GDIR_k);
            //@ assert G_A_k:             EqualBits(stream, pos + 62,  pos + 70,  p->G_A_k);
...
int Gradient_Profile_Core_1_Decoder(Bitstream* stream, Gradient_Profile_Core_1* p)
{
	...
	/*@
	  requires D_GRADIENT_k:   stream->bitpos == pos + 46;
	  assigns        	   stream->bitpos;
	  assigns		   p->D_GRADIENT_k;
	  ensures  D_GRADIENT_k:   stream->bitpos == pos + 61;
	  ensures  D_GRADIENT_k:   EqualBits(stream, pos + 46, pos + 61, p->D_GRADIENT_k);
	  ensures  D_GRADIENT_k:   UpperBitsNotSet(p->D_GRADIENT_k, 15);
	*/
	{ p->D_GRADIENT_k		= Bitstream_Read(stream, 15); }

	/*@
	  requires Q_GDIR_k:       stream->bitpos == pos + 61;
	  assigns        	   stream->bitpos;
	  assigns		   p->Q_GDIR_k;
	  ensures  Q_GDIR_k:       stream->bitpos == pos + 62;
	  ensures  Q_GDIR_k:       EqualBits(stream, pos + 61, pos + 62, p->Q_GDIR_k);
	  ensures  Q_GDIR_k:       UpperBitsNotSet(p->Q_GDIR_k, 1);
	*/
	{ p->Q_GDIR_k		= Bitstream_Read(stream, 1); }

	/*@
	  requires G_A_k:          stream->bitpos == pos + 62;
	  assigns        	   stream->bitpos;
	  assigns		   p->G_A_k;
	  ensures  G_A_k:          stream->bitpos == pos + 70;
	  ensures  G_A_k:          EqualBits(stream, pos + 62, pos + 70, p->G_A_k);
