...
int National_Values_Encoder(Bitstream* stream, const National_Values_Core* p)
{
    if (NormalBitstream(stream, NATIONAL_VALUES_CORE_BITSIZE))
    {
        if (National_Values_UpperBitsNotSet(p))
        {
            ...
            //@ assert V_NVSHUNT:         EqualBits(stream, pos + 42,  pos + 49,  p->V_NVSHUNT);
            //@ assert V_NVSTFF:          EqualBits(stream, pos + 49,  pos + 56,  p->V_NVSTFF);
            //@ assert V_NVONSIGHT:       EqualBits(stream, pos + 56,  pos + 63,  p->V_NVONSIGHT);
            //@ assert V_NVLIMSUPERV:     EqualBits(stream, pos + 63,  pos + 70,  p->V_NVLIMSUPERV);
            //@ assert V_NVUNFIT:         EqualBits(stream, pos + 70,  pos + 77,  p->V_NVUNFIT);
            //@ assert V_NVREL:           EqualBits(stream, pos + 77,  pos + 84,  p->V_NVREL);
            //@ assert D_NVROLL:          EqualBits(stream, pos + 84,  pos + 99,  p->D_NVROLL);
            //@ assert Q_NVSBTSMPERM:     EqualBits(stream, pos + 99,  pos + 100, p->Q_NVSBTSMPERM);
            //@ assert Q_NVEMRRLS:        EqualBits(stream, pos + 100, pos + 101, p->Q_NVEMRRLS);
            //@ assert Q_NVGUIPERM:       EqualBits(stream, pos + 101, pos + 102, p->Q_NVGUIPERM);
            //@ assert Q_NVSBFBPERM:      EqualBits(stream, pos + 102, pos + 103, p->Q_NVSBFBPERM);
            //@ assert Q_NVINHSMICPERM:   EqualBits(stream, pos + 103, pos + 104, p->Q_NVINHSMICPERM);
            //@ assert V_NVALLOWOVTRP:    EqualBits(stream, pos + 104, pos + 111, p->V_NVALLOWOVTRP);
            //@ assert V_NVSUPOVTRP:      EqualBits(stream, pos + 111, pos + 118, p->V_NVSUPOVTRP);
            //@ assert D_NVOVTRP:         EqualBits(stream, pos + 118, pos + 133, p->D_NVOVTRP);
            //@ assert T_NVOVTRP:         EqualBits(stream, pos + 133, pos + 141, p->T_NVOVTRP);
            //@ assert D_NVPOTRP:         EqualBits(stream, pos + 141, pos + 156, p->D_NVPOTRP);
            //@ assert M_NVCONTACT:       EqualBits(stream, pos + 156, pos + 158, p->M_NVCONTACT);
            //@ assert T_NVCONTACT:       EqualBits(stream, pos + 158, pos + 166, p->T_NVCONTACT);
            //@ assert M_NVDERUN:         EqualBits(stream, pos + 166, pos + 167, p->M_NVDERUN);
            //@ assert D_NVSTFF:          EqualBits(stream, pos + 167, pos + 182, p->D_NVSTFF);
            //@ assert Q_NVDRIVER_ADHES:  EqualBits(stream, pos + 182, pos + 183, p->Q_NVDRIVER_ADHES);
            //@ assert A_NVMAXREDADH1:    EqualBits(stream, pos + 183, pos + 189, p->A_NVMAXREDADH1);
            //@ assert A_NVMAXREDADH2:    EqualBits(stream, pos + 189, pos + 195, p->A_NVMAXREDADH2);
            //@ assert A_NVMAXREDADH3:    EqualBits(stream, pos + 195, pos + 201, p->A_NVMAXREDADH3);
            //@ assert Q_NVLOCACC:        EqualBits(stream, pos + 201, pos + 207, p->Q_NVLOCACC);
            //@ assert M_NVAVADH:         EqualBits(stream, pos + 207, pos + 212, p->M_NVAVADH);
            //@ assert M_NVEBCL:          EqualBits(stream, pos + 212, pos + 216, p->M_NVEBCL);
            //@ assert Q_NVKINT:          EqualBits(stream, pos + 216, pos + 217, p->Q_NVKINT);
...
int National_Values_Decoder(Bitstream* stream, National_Values_Core* p)
{
	...
	*/
	...
	/*@
	  requires V_NVSHUNT:      stream->bitpos == pos + 42;
	  assigns        	   stream->bitpos;
	  assigns		   p->V_NVSHUNT;
	  ensures  V_NVSHUNT:      stream->bitpos == pos + 49;
	  ensures  V_NVSHUNT:      EqualBits(stream, pos + 42, pos + 49, p->V_NVSHUNT);
	  ensures  V_NVSHUNT:      UpperBitsNotSet(p->V_NVSHUNT, 7);
	*/
	{ p->V_NVSHUNT		= Bitstream_Read(stream, 7); }

	/*@
	  requires V_NVSTFF:       stream->bitpos == pos + 49;
	  assigns        	   stream->bitpos;
	  assigns		   p->V_NVSTFF;
	  ensures  V_NVSTFF:       stream->bitpos == pos + 56;
	  ensures  V_NVSTFF:       EqualBits(stream, pos + 49, pos + 56, p->V_NVSTFF);
	  ensures  V_NVSTFF:       UpperBitsNotSet(p->V_NVSTFF, 7);
	*/
	{ p->V_NVSTFF		= Bitstream_Read(stream, 7); }

	/*@
	  requires V_NVONSIGHT:    stream->bitpos == pos + 56;
	  assigns        	   stream->bitpos;
	  assigns		   p->V_NVONSIGHT;
	  ensures  V_NVONSIGHT:    stream->bitpos == pos + 63;
	  ensures  V_NVONSIGHT:    EqualBits(stream, pos + 56, pos + 63, p->V_NVONSIGHT);
	  ensures  V_NVONSIGHT:    UpperBitsNotSet(p->V_NVONSIGHT, 7);
	*/
	{ p->V_NVONSIGHT		= Bitstream_Read(stream, 7); }

	/*@
	  requires V_NVLIMSUPERV:  stream->bitpos == pos + 63;
	  assigns        	   stream->bitpos;
	  assigns		   p->V_NVLIMSUPERV;
	  ensures  V_NVLIMSUPERV:  stream->bitpos == pos + 70;
	  ensures  V_NVLIMSUPERV:  EqualBits(stream, pos + 63, pos + 70, p->V_NVLIMSUPERV);
	  ensures  V_NVLIMSUPERV:  UpperBitsNotSet(p->V_NVLIMSUPERV, 7);
	*/
	{ p->V_NVLIMSUPERV		= Bitstream_Read(stream, 7); }

	/*@
	  requires V_NVUNFIT:      stream->bitpos == pos + 70;
	  assigns        	   stream->bitpos;
	  assigns		   p->V_NVUNFIT;
	  ensures  V_NVUNFIT:      stream->bitpos == pos + 77;
	  ensures  V_NVUNFIT:      EqualBits(stream, pos + 70, pos + 77, p->V_NVUNFIT);
	  ensures  V_NVUNFIT:      UpperBitsNotSet(p->V_NVUNFIT, 7);
	*/
	{ p->V_NVUNFIT		= Bitstream_Read(stream, 7); }

	/*@
	  requires V_NVREL:        stream->bitpos == pos + 77;
	  assigns        	   stream->bitpos;
	  assigns		   p->V_NVREL;
	  ensures  V_NVREL:        stream->bitpos == pos + 84;
	  ensures  V_NVREL:        EqualBits(stream, pos + 77, pos + 84, p->V_NVREL);
	  ensures  V_NVREL:        UpperBitsNotSet(p->V_NVREL, 7);
	*/
	{ p->V_NVREL		= Bitstream_Read(stream, 7); }

	/*@
	  requires D_NVROLL:       stream->bitpos == pos + 84;
	  assigns        	   stream->bitpos;
	  assigns		   p->D_NVROLL;
	  ensures  D_NVROLL:       stream->bitpos == pos + 99;
	  ensures  D_NVROLL:       EqualBits(stream, pos + 84, pos + 99, p->D_NVROLL);
	  ensures  D_NVROLL:       UpperBitsNotSet(p->D_NVROLL, 15);
	*/
	{ p->D_NVROLL		= Bitstream_Read(stream, 15); }

	/*@
	  requires Q_NVSBTSMPERM:  stream->bitpos == pos + 99;
	  assigns        	   stream->bitpos;
	  assigns		   p->Q_NVSBTSMPERM;
	  ensures  Q_NVSBTSMPERM:  stream->bitpos == pos + 100;
	  ensures  Q_NVSBTSMPERM:  EqualBits(stream, pos + 99, pos + 100, p->Q_NVSBTSMPERM);
	  ensures  Q_NVSBTSMPERM:  UpperBitsNotSet(p->Q_NVSBTSMPERM, 1);
	*/
	{ p->Q_NVSBTSMPERM		= Bitstream_Read(stream, 1); }

	/*@
	  requires Q_NVEMRRLS:     stream->bitpos == pos + 100;
	  assigns        	   stream->bitpos;
	  assigns		   p->Q_NVEMRRLS;
	  ensures  Q_NVEMRRLS:     stream->bitpos == pos + 101;
	  ensures  Q_NVEMRRLS:     EqualBits(stream, pos + 100, pos + 101, p->Q_NVEMRRLS);
	  ensures  Q_NVEMRRLS:     UpperBitsNotSet(p->Q_NVEMRRLS, 1);
	*/
	{ p->Q_NVEMRRLS		= Bitstream_Read(stream, 1); }

	/*@
	  requires Q_NVGUIPERM:    stream->bitpos == pos + 101;
	  assigns        	   stream->bitpos;
	  assigns		   p->Q_NVGUIPERM;
	  ensures  Q_NVGUIPERM:    stream->bitpos == pos + 102;
	  ensures  Q_NVGUIPERM:    EqualBits(stream, pos + 101, pos + 102, p->Q_NVGUIPERM);
	  ensures  Q_NVGUIPERM:    UpperBitsNotSet(p->Q_NVGUIPERM, 1);
	*/
	{ p->Q_NVGUIPERM		= Bitstream_Read(stream, 1); }

	/*@
	  requires Q_NVSBFBPERM:   stream->bitpos == pos + 102;
	  assigns        	   stream->bitpos;
	  assigns		   p->Q_NVSBFBPERM;
	  ensures  Q_NVSBFBPERM:   stream->bitpos == pos + 103;
	  ensures  Q_NVSBFBPERM:   EqualBits(stream, pos + 102, pos + 103, p->Q_NVSBFBPERM);
	  ensures  Q_NVSBFBPERM:   UpperBitsNotSet(p->Q_NVSBFBPERM, 1);
	*/
	{ p->Q_NVSBFBPERM		= Bitstream_Read(stream, 1); }

	/*@
	  requires Q_NVINHSMICPERM: stream->bitpos == pos + 103;
	  assigns        	   stream->bitpos;
	  assigns		   p->Q_NVINHSMICPERM;
	  ensures  Q_NVINHSMICPERM: stream->bitpos == pos + 104;
	  ensures  Q_NVINHSMICPERM: EqualBits(stream, pos + 103, pos + 104, p->Q_NVINHSMICPERM);
	  ensures  Q_NVINHSMICPERM: UpperBitsNotSet(p->Q_NVINHSMICPERM, 1);
	*/
	{ p->Q_NVINHSMICPERM		= Bitstream_Read(stream, 1); }

	/*@
	  requires V_NVALLOWOVTRP: stream->bitpos == pos + 104;
	  assigns        	   stream->bitpos;
	  assigns		   p->V_NVALLOWOVTRP;
	  ensures  V_NVALLOWOVTRP: stream->bitpos == pos + 111;
	  ensures  V_NVALLOWOVTRP: EqualBits(stream, pos + 104, pos + 111, p->V_NVALLOWOVTRP);
	  ensures  V_NVALLOWOVTRP: UpperBitsNotSet(p->V_NVALLOWOVTRP, 7);
	*/
	{ p->V_NVALLOWOVTRP		= Bitstream_Read(stream, 7); }

	/*@
	  requires V_NVSUPOVTRP:   stream->bitpos == pos + 111;
	  assigns        	   stream->bitpos;
	  assigns		   p->V_NVSUPOVTRP;
	  ensures  V_NVSUPOVTRP:   stream->bitpos == pos + 118;
	  ensures  V_NVSUPOVTRP:   EqualBits(stream, pos + 111, pos + 118, p->V_NVSUPOVTRP);
	  ensures  V_NVSUPOVTRP:   UpperBitsNotSet(p->V_NVSUPOVTRP, 7);
	*/
	{ p->V_NVSUPOVTRP		= Bitstream_Read(stream, 7); }

	/*@
	  requires D_NVOVTRP:      stream->bitpos == pos + 118;
	  assigns        	   stream->bitpos;
	  assigns		   p->D_NVOVTRP;
	  ensures  D_NVOVTRP:      stream->bitpos == pos + 133;
	  ensures  D_NVOVTRP:      EqualBits(stream, pos + 118, pos + 133, p->D_NVOVTRP);
	  ensures  D_NVOVTRP:      UpperBitsNotSet(p->D_NVOVTRP, 15);
	*/
	{ p->D_NVOVTRP		= Bitstream_Read(stream, 15); }

	/*@
	  requires T_NVOVTRP:      stream->bitpos == pos + 133;
	  assigns        	   stream->bitpos;
	  assigns		   p->T_NVOVTRP;
	  ensures  T_NVOVTRP:      stream->bitpos == pos + 141;
	  ensures  T_NVOVTRP:      EqualBits(stream, pos + 133, pos + 141, p->T_NVOVTRP);
	  ensures  T_NVOVTRP:      UpperBitsNotSet(p->T_NVOVTRP, 8);
	*/
	{ p->T_NVOVTRP		= Bitstream_Read(stream, 8); }

	/*@
	  requires D_NVPOTRP:      stream->bitpos == pos + 141;
	  assigns        	   stream->bitpos;
	  assigns		   p->D_NVPOTRP;
	  ensures  D_NVPOTRP:      stream->bitpos == pos + 156;
	  ensures  D_NVPOTRP:      EqualBits(stream, pos + 141, pos + 156, p->D_NVPOTRP);
	  ensures  D_NVPOTRP:      UpperBitsNotSet(p->D_NVPOTRP, 15);
	*/
	{ p->D_NVPOTRP		= Bitstream_Read(stream, 15); }

	/*@
	  requires M_NVCONTACT:    stream->bitpos == pos + 156;
	  assigns        	   stream->bitpos;
	  assigns		   p->M_NVCONTACT;
	  ensures  M_NVCONTACT:    stream->bitpos == pos + 158;
	  ensures  M_NVCONTACT:    EqualBits(stream, pos + 156, pos + 158, p->M_NVCONTACT);
	  ensures  M_NVCONTACT:    UpperBitsNotSet(p->M_NVCONTACT, 2);
	*/
	{ p->M_NVCONTACT		= Bitstream_Read(stream, 2); }

	/*@
	  requires T_NVCONTACT:    stream->bitpos == pos + 158;
	  assigns        	   stream->bitpos;
	  assigns		   p->T_NVCONTACT;
	  ensures  T_NVCONTACT:    stream->bitpos == pos + 166;
	  ensures  T_NVCONTACT:    EqualBits(stream, pos + 158, pos + 166, p->T_NVCONTACT);
	  ensures  T_NVCONTACT:    UpperBitsNotSet(p->T_NVCONTACT, 8);
	*/
	{ p->T_NVCONTACT		= Bitstream_Read(stream, 8); }

	/*@
	  requires M_NVDERUN:      stream->bitpos == pos + 166;
	  assigns        	   stream->bitpos;
	  assigns		   p->M_NVDERUN;
	  ensures  M_NVDERUN:      stream->bitpos == pos + 167;
	  ensures  M_NVDERUN:      EqualBits(stream, pos + 166, pos + 167, p->M_NVDERUN);
	  ensures  M_NVDERUN:      UpperBitsNotSet(p->M_NVDERUN, 1);
	*/
	{ p->M_NVDERUN		= Bitstream_Read(stream, 1); }

	/*@
	  requires D_NVSTFF:       stream->bitpos == pos + 167;
	  assigns        	   stream->bitpos;
	  assigns		   p->D_NVSTFF;
	  ensures  D_NVSTFF:       stream->bitpos == pos + 182;
	  ensures  D_NVSTFF:       EqualBits(stream, pos + 167, pos + 182, p->D_NVSTFF);
	  ensures  D_NVSTFF:       UpperBitsNotSet(p->D_NVSTFF, 15);
	*/
	{ p->D_NVSTFF		= Bitstream_Read(stream, 15); }

	/*@
	  requires Q_NVDRIVER_ADHES: stream->bitpos == pos + 182;
	  assigns        	   stream->bitpos;
	  assigns		   p->Q_NVDRIVER_ADHES;
	  ensures  Q_NVDRIVER_ADHES: stream->bitpos == pos + 183;
	  ensures  Q_NVDRIVER_ADHES: EqualBits(stream, pos + 182, pos + 183, p->Q_NVDRIVER_ADHES);
	  ensures  Q_NVDRIVER_ADHES: UpperBitsNotSet(p->Q_NVDRIVER_ADHES, 1);
	*/
	{ p->Q_NVDRIVER_ADHES		= Bitstream_Read(stream, 1); }

	/*@
	  requires A_NVMAXREDADH1: stream->bitpos == pos + 183;
	  assigns        	   stream->bitpos;
	  assigns		   p->A_NVMAXREDADH1;
	  ensures  A_NVMAXREDADH1: stream->bitpos == pos + 189;
	  ensures  A_NVMAXREDADH1: EqualBits(stream, pos + 183, pos + 189, p->A_NVMAXREDADH1);
	  ensures  A_NVMAXREDADH1: UpperBitsNotSet(p->A_NVMAXREDADH1, 6);
	*/
	{ p->A_NVMAXREDADH1		= Bitstream_Read(stream, 6); }

	/*@
	  requires A_NVMAXREDADH2: stream->bitpos == pos + 189;
	  assigns        	   stream->bitpos;
	  assigns		   p->A_NVMAXREDADH2;
	  ensures  A_NVMAXREDADH2: stream->bitpos == pos + 195;
	  ensures  A_NVMAXREDADH2: EqualBits(stream, pos + 189, pos + 195, p->A_NVMAXREDADH2);
	  ensures  A_NVMAXREDADH2: UpperBitsNotSet(p->A_NVMAXREDADH2, 6);
	*/
	{ p->A_NVMAXREDADH2		= Bitstream_Read(stream, 6); }

	/*@
	  requires A_NVMAXREDADH3: stream->bitpos == pos + 195;
	  assigns        	   stream->bitpos;
	  assigns		   p->A_NVMAXREDADH3;
	  ensures  A_NVMAXREDADH3: stream->bitpos == pos + 201;
	  ensures  A_NVMAXREDADH3: EqualBits(stream, pos + 195, pos + 201, p->A_NVMAXREDADH3);
	  ensures  A_NVMAXREDADH3: UpperBitsNotSet(p->A_NVMAXREDADH3, 6);
	*/
	{ p->A_NVMAXREDADH3		= Bitstream_Read(stream, 6); }

	/*@
	  requires Q_NVLOCACC:     stream->bitpos == pos + 201;
	  assigns        	   stream->bitpos;
	  assigns		   p->Q_NVLOCACC;
	  ensures  Q_NVLOCACC:     stream->bitpos == pos + 207;
	  ensures  Q_NVLOCACC:     EqualBits(stream, pos + 201, pos + 207, p->Q_NVLOCACC);
	  ensures  Q_NVLOCACC:     UpperBitsNotSet(p->Q_NVLOCACC, 6);
	*/
	{ p->Q_NVLOCACC		= Bitstream_Read(stream, 6); }

	/*@
	  requires M_NVAVADH:      stream->bitpos == pos + 207;
	  assigns        	   stream->bitpos;
	  assigns		   p->M_NVAVADH;
	  ensures  M_NVAVADH:      stream->bitpos == pos + 212;
	  ensures  M_NVAVADH:      EqualBits(stream, pos + 207, pos + 212, p->M_NVAVADH);
	  ensures  M_NVAVADH:      UpperBitsNotSet(p->M_NVAVADH, 5);
	*/
	{ p->M_NVAVADH		= Bitstream_Read(stream, 5); }

	/*@
	  requires M_NVEBCL:       stream->bitpos == pos + 212;
	  assigns        	   stream->bitpos;
	  assigns		   p->M_NVEBCL;
	  ensures  M_NVEBCL:       stream->bitpos == pos + 216;
	  ensures  M_NVEBCL:       EqualBits(stream, pos + 212, pos + 216, p->M_NVEBCL);
	  ensures  M_NVEBCL:       UpperBitsNotSet(p->M_NVEBCL, 4);
	*/
	{ p->M_NVEBCL		= Bitstream_Read(stream, 4); }

	/*@
	  requires Q_NVKINT:       stream->bitpos == pos + 216;
	  assigns        	   stream->bitpos;
	  assigns		   p->Q_NVKINT;
	  ensures  Q_NVKINT:       stream->bitpos == pos + 217;
	  ensures  Q_NVKINT:       EqualBits(stream, pos + 216, pos + 217, p->Q_NVKINT);
