...
int Packet_for_sending_fixed_text_messages_Decoder(Bitstream* stream, Packet_for_sending_fixed_text_messages_Core* p)
{
	...
	*/
	...
	/*@
	  requires Q_TEXT:         stream->bitpos == pos + 126;
	  assigns        	   stream->bitpos;
	  assigns		   p->Q_TEXT;
	  ensures  Q_TEXT:         stream->bitpos == pos + 134;
	  ensures  Q_TEXT:         EqualBits(stream, pos + 126, pos + 134, p->Q_TEXT);
	  ensures  Q_TEXT:         UpperBitsNotSet(p->Q_TEXT, 8);
	*/
	{ p->Q_TEXT		= Bitstream_Read(stream, 8); }
        ...
        //@ assert Q_TEXT:            UpperBitsNotSet(p->Q_TEXT,            8);
