...
int Level_1_Movement_Authority_Encoder(Bitstream* stream, const Level_1_Movement_Authority_Core* p)
{
    if (NormalBitstream(stream, LEVEL_1_MOVEMENT_AUTHORITY_CORE_BITSIZE))
    {
        if (Level_1_Movement_Authority_UpperBitsNotSet(p))
        {
            ...
            //@ assert L_ENDSECTION:      EqualBits(stream, pos + 87,  pos + 102, p->L_ENDSECTION);
            //@ assert Q_SECTIONTIMER:    EqualBits(stream, pos + 102, pos + 103, p->Q_SECTIONTIMER);
            //@ assert T_SECTIONTIMER:    EqualBits(stream, pos + 103, pos + 113, p->T_SECTIONTIMER);
            //@ assert D_SECTIONTIMERSTOPLOC: EqualBits(stream, pos + 113, pos + 128, p->D_SECTIONTIMERSTOPLOC);
            //@ assert Q_ENDTIMER:        EqualBits(stream, pos + 128, pos + 129, p->Q_ENDTIMER);
            //@ assert T_ENDTIMER:        EqualBits(stream, pos + 129, pos + 139, p->T_ENDTIMER);
            //@ assert D_ENDTIMERSTARTLOC: EqualBits(stream, pos + 139, pos + 154, p->D_ENDTIMERSTARTLOC);
            //@ assert Q_DANGERPOINT:     EqualBits(stream, pos + 154, pos + 155, p->Q_DANGERPOINT);
            //@ assert D_DP:              EqualBits(stream, pos + 155, pos + 170, p->D_DP);
            //@ assert V_RELEASEDP:       EqualBits(stream, pos + 170, pos + 177, p->V_RELEASEDP);
            //@ assert Q_OVERLAP:         EqualBits(stream, pos + 177, pos + 178, p->Q_OVERLAP);
            //@ assert D_STARTOL:         EqualBits(stream, pos + 178, pos + 193, p->D_STARTOL);
            //@ assert T_OL:              EqualBits(stream, pos + 193, pos + 203, p->T_OL);
            //@ assert D_OL:              EqualBits(stream, pos + 203, pos + 218, p->D_OL);
            //@ assert V_RELEASEOL:       EqualBits(stream, pos + 218, pos + 225, p->V_RELEASEOL);
...
int Level_1_Movement_Authority_Decoder(Bitstream* stream, Level_1_Movement_Authority_Core* p)
{
	...
	*/
	...
	/*@
	  requires L_ENDSECTION:   stream->bitpos == pos + 87;
	  assigns        	   stream->bitpos;
	  assigns		   p->L_ENDSECTION;
	  ensures  L_ENDSECTION:   stream->bitpos == pos + 102;
	  ensures  L_ENDSECTION:   EqualBits(stream, pos + 87, pos + 102, p->L_ENDSECTION);
	  ensures  L_ENDSECTION:   UpperBitsNotSet(p->L_ENDSECTION, 15);
	*/
	{ p->L_ENDSECTION		= Bitstream_Read(stream, 15); }

	/*@
	  requires Q_SECTIONTIMER: stream->bitpos == pos + 102;
	  assigns        	   stream->bitpos;
	  assigns		   p->Q_SECTIONTIMER;
	  ensures  Q_SECTIONTIMER: stream->bitpos == pos + 103;
	  ensures  Q_SECTIONTIMER: EqualBits(stream, pos + 102, pos + 103, p->Q_SECTIONTIMER);
	  ensures  Q_SECTIONTIMER: UpperBitsNotSet(p->Q_SECTIONTIMER, 1);
	*/
	{ p->Q_SECTIONTIMER		= Bitstream_Read(stream, 1); }

	/*@
	  requires T_SECTIONTIMER: stream->bitpos == pos + 103;
	  assigns        	   stream->bitpos;
	  assigns		   p->T_SECTIONTIMER;
	  ensures  T_SECTIONTIMER: stream->bitpos == pos + 113;
	  ensures  T_SECTIONTIMER: EqualBits(stream, pos + 103, pos + 113, p->T_SECTIONTIMER);
	  ensures  T_SECTIONTIMER: UpperBitsNotSet(p->T_SECTIONTIMER, 10);
	*/
	{ p->T_SECTIONTIMER		= Bitstream_Read(stream, 10); }

	/*@
	  requires D_SECTIONTIMERSTOPLOC: stream->bitpos == pos + 113;
	  assigns        	   stream->bitpos;
	  assigns		   p->D_SECTIONTIMERSTOPLOC;
	  ensures  D_SECTIONTIMERSTOPLOC: stream->bitpos == pos + 128;
	  ensures  D_SECTIONTIMERSTOPLOC: EqualBits(stream, pos + 113, pos + 128, p->D_SECTIONTIMERSTOPLOC);
	  ensures  D_SECTIONTIMERSTOPLOC: UpperBitsNotSet(p->D_SECTIONTIMERSTOPLOC, 15);
	*/
	{ p->D_SECTIONTIMERSTOPLOC		= Bitstream_Read(stream, 15); }

	/*@
	  requires Q_ENDTIMER:     stream->bitpos == pos + 128;
	  assigns        	   stream->bitpos;
	  assigns		   p->Q_ENDTIMER;
	  ensures  Q_ENDTIMER:     stream->bitpos == pos + 129;
	  ensures  Q_ENDTIMER:     EqualBits(stream, pos + 128, pos + 129, p->Q_ENDTIMER);
	  ensures  Q_ENDTIMER:     UpperBitsNotSet(p->Q_ENDTIMER, 1);
	*/
	{ p->Q_ENDTIMER		= Bitstream_Read(stream, 1); }

	/*@
	  requires T_ENDTIMER:     stream->bitpos == pos + 129;
	  assigns        	   stream->bitpos;
	  assigns		   p->T_ENDTIMER;
	  ensures  T_ENDTIMER:     stream->bitpos == pos + 139;
	  ensures  T_ENDTIMER:     EqualBits(stream, pos + 129, pos + 139, p->T_ENDTIMER);
	  ensures  T_ENDTIMER:     UpperBitsNotSet(p->T_ENDTIMER, 10);
	*/
	{ p->T_ENDTIMER		= Bitstream_Read(stream, 10); }

	/*@
	  requires D_ENDTIMERSTARTLOC: stream->bitpos == pos + 139;
	  assigns        	   stream->bitpos;
	  assigns		   p->D_ENDTIMERSTARTLOC;
	  ensures  D_ENDTIMERSTARTLOC: stream->bitpos == pos + 154;
	  ensures  D_ENDTIMERSTARTLOC: EqualBits(stream, pos + 139, pos + 154, p->D_ENDTIMERSTARTLOC);
	  ensures  D_ENDTIMERSTARTLOC: UpperBitsNotSet(p->D_ENDTIMERSTARTLOC, 15);
	*/
	{ p->D_ENDTIMERSTARTLOC		= Bitstream_Read(stream, 15); }

	/*@
	  requires Q_DANGERPOINT:  stream->bitpos == pos + 154;
	  assigns        	   stream->bitpos;
	  assigns		   p->Q_DANGERPOINT;
	  ensures  Q_DANGERPOINT:  stream->bitpos == pos + 155;
	  ensures  Q_DANGERPOINT:  EqualBits(stream, pos + 154, pos + 155, p->Q_DANGERPOINT);
	  ensures  Q_DANGERPOINT:  UpperBitsNotSet(p->Q_DANGERPOINT, 1);
	*/
	{ p->Q_DANGERPOINT		= Bitstream_Read(stream, 1); }

	/*@
	  requires D_DP:           stream->bitpos == pos + 155;
	  assigns        	   stream->bitpos;
	  assigns		   p->D_DP;
	  ensures  D_DP:           stream->bitpos == pos + 170;
	  ensures  D_DP:           EqualBits(stream, pos + 155, pos + 170, p->D_DP);
	  ensures  D_DP:           UpperBitsNotSet(p->D_DP, 15);
	*/
	{ p->D_DP		= Bitstream_Read(stream, 15); }

	/*@
	  requires V_RELEASEDP:    stream->bitpos == pos + 170;
	  assigns        	   stream->bitpos;
	  assigns		   p->V_RELEASEDP;
	  ensures  V_RELEASEDP:    stream->bitpos == pos + 177;
	  ensures  V_RELEASEDP:    EqualBits(stream, pos + 170, pos + 177, p->V_RELEASEDP);
	  ensures  V_RELEASEDP:    UpperBitsNotSet(p->V_RELEASEDP, 7);
	*/
	{ p->V_RELEASEDP		= Bitstream_Read(stream, 7); }

	/*@
	  requires Q_OVERLAP:      stream->bitpos == pos + 177;
	  assigns        	   stream->bitpos;
	  assigns		   p->Q_OVERLAP;
	  ensures  Q_OVERLAP:      stream->bitpos == pos + 178;
	  ensures  Q_OVERLAP:      EqualBits(stream, pos + 177, pos + 178, p->Q_OVERLAP);
	  ensures  Q_OVERLAP:      UpperBitsNotSet(p->Q_OVERLAP, 1);
	*/
	{ p->Q_OVERLAP		= Bitstream_Read(stream, 1); }

	/*@
	  requires D_STARTOL:      stream->bitpos == pos + 178;
	  assigns        	   stream->bitpos;
	  assigns		   p->D_STARTOL;
	  ensures  D_STARTOL:      stream->bitpos == pos + 193;
	  ensures  D_STARTOL:      EqualBits(stream, pos + 178, pos + 193, p->D_STARTOL);
	  ensures  D_STARTOL:      UpperBitsNotSet(p->D_STARTOL, 15);
	*/
	{ p->D_STARTOL		= Bitstream_Read(stream, 15); }

	/*@
	  requires T_OL:           stream->bitpos == pos + 193;
	  assigns        	   stream->bitpos;
	  assigns		   p->T_OL;
	  ensures  T_OL:           stream->bitpos == pos + 203;
	  ensures  T_OL:           EqualBits(stream, pos + 193, pos + 203, p->T_OL);
	  ensures  T_OL:           UpperBitsNotSet(p->T_OL, 10);
	*/
	{ p->T_OL		= Bitstream_Read(stream, 10); }

	/*@
	  requires D_OL:           stream->bitpos == pos + 203;
	  assigns        	   stream->bitpos;
	  assigns		   p->D_OL;
	  ensures  D_OL:           stream->bitpos == pos + 218;
	  ensures  D_OL:           EqualBits(stream, pos + 203, pos + 218, p->D_OL);
	  ensures  D_OL:           UpperBitsNotSet(p->D_OL, 15);
	*/
	{ p->D_OL		= Bitstream_Read(stream, 15); }

	/*@
	  requires V_RELEASEOL:    stream->bitpos == pos + 218;
	  assigns        	   stream->bitpos;
	  assigns		   p->V_RELEASEOL;
	  ensures  V_RELEASEOL:    stream->bitpos == pos + 225;
	  ensures  V_RELEASEOL:    EqualBits(stream, pos + 218, pos + 225, p->V_RELEASEOL);
