...
/*@
    ...
    predicate Invariant(Level_1_Movement_Authority_Core_1* p) =
      Invariant(p->L_SECTION)         &&
      Invariant(p->Q_SECTIONTIMER)    &&
      Invariant(p->T_SECTIONTIMER)    &&
      Invariant(p->D_SECTIONTIMERSTOPLOC);

    predicate ZeroInitialized(Level_1_Movement_Authority_Core_1* p) =
...
int Level_1_Movement_Authority_Core_1_Encode_Int(Packet_Info* data, kcg_int* stream, const Level_1_Movement_Authority_Core_1* p);

int Level_1_Movement_Authority_Core_1_Decode_Int(Packet_Info* data, const kcg_int* stream, Level_1_Movement_Authority_Core_1* p);
...
inline int encode(Packet_Info& data, kcg_int* stream, const Level_1_Movement_Authority_Core_1& p)
{
    std::cerr << "encode int function not implemented for packet 12 yet." << std::endl;

    return Level_1_Movement_Authority_Core_1_Encode_Int(&data, stream, &p);
}

inline int decode(Packet_Info& data, const kcg_int* stream, Level_1_Movement_Authority_Core_1& p)
{
